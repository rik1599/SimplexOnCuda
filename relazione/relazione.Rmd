...
Table: Risultati sperimentali dei diversi kernel, con la configurazione di lancio ottimale (blocco: 32*32, grid: (1, roof(24.588/32))) ed ottenuti con una scheda Geforce MX250
...
Anche qui vengono utilizzati due CUDA Stream: il primo esegue *cudaMemsetAsync* sugli ultimi $m$ elementi del vettore dei costi per impostare i valori a 0,
il secondo copia il vettore della funzione obiettivo dalla memoria host ai primi $n$ elementi del vettore dei costi con *cudaMemcpyAsync* e nega il segno degli elementi copiati (utilizzando lo schema già visto nella sezione precedente).
...
Per fare ciò è stata implementata una reduction (sempre intra-warp a due fasi perchè non esiste *atomicMax* per i valori double) per ottenere il valore massimo di un vettore. Se il valore massimo di un vettore è $\leq 0$, allora devono per forza esserlo anche tutti gli altri.
...
Prendendo in considerazione che per una matrice da $32 \times 32$ elementi il pattern migliore è quello che ad ogni thread assegna un valore, per calcolare il numero di blocchi necessari data una dimensione della matrice è stata calcolata una retta tra i punti $(1,32)$ e $(8192,16)$ ottenendo così l'equazione $$ y = \left\lceil \frac{x+512}{544} \right \rceil $$
...
Per quanto riguarda l'aggiornamento del tableau, per effettuare il calcolo $T[i][j] - \frac{C_p[i]}{p}R_p[j]$ viene utilizzata la funzione *fma* della libreria math di CUDA\footnote{Questa funzione riduce calcoli del tipo $A*B+C$ in una sola operazione in virgola mobile}. Il calcolo viene così ridotto a due operazioni in virgola mobile (divisione e fma) invece che tre.
...
1. Si copia il primo valore del vettore dei costi del tableu per ottenere il valore ottimo della funzione obiettivo,
...
La spiegazione di questa caratteristica è da ricercarsi nel funzionamento pratico dell'algoritmo: durante il ***ciclo di solve*** un passo fondamentale dell'algoritmo è l'aggiornamento del tableu, tale tableu non ha una dimensione pari a $n^{\circ}variabili \times n^{\circ}vincoli$, bensì pari a $(n^{\circ}variabili + 2n^{\circ}vincoli) \times n^{\circ}vincoli$ nel caso della fase 1, mentre $(n^{\circ}variabili + n^{\circ}vincoli) \times n^{\circ}vincoli$ nel caso della fase 2.
...
```{r, echo=FALSE, warning=FALSE}
  ...
  points(phaseTwoMean_ElemNumber$cells, phaseTwoMean_ElemNumber$values, col="red")
...
Si nota come il tempo medio necessario per ciclo di solve sembra cresce linearmente all'aumentare della dimensione effettiva del tableu.
...
<!-- Inserire qualche informazione in più sulle schede (Architettura, n° di stream multiprocessor, ecc...) -->
...
### Confronto risultati
Dato che il tempo medio per ciclo di solve dipende principalmente dalla dimensione reale del tableu i confronti tra schede diverse verranno effettuati in base a tale parametro. 

Visto inoltre che questa varia in base alla fase in cui ci si trova solo una delle due verrà presa in considerazione, in particolare si è scelta la fase 1 per via del maggiore numero di cicli eseguiti che permettono di ottenere dei tempi medi più attendibili. <!--  forse dato che si utilizza la dimensione effettiva si potrebbe unire fase 1 e 2 -->
...
par(mfrow = c(1,1))
plot(data = difference, difference~cells  , col = "black", main = "Differenza di tempo medio per ciclo di solve tra \nMX250 e RTX2070 Super")
...
Dal primo grafico è possibile notare come la differenza di tempo medio tra la MX250 e la RTX2070 Super cresca limearmente all'aumentare della dimensione effettiva del problema, ovvero all'aumentare della dimensione di celle effettive del tableu, passando da `r round(difference_256x256/1000, digits = 2)`ms nel caso di problemi di dimensione 256x256 a `r round(difference_8192x8192/1000, digits = 2)`ms per quelli di dimensione 8192x8192.
...
legend("bottom", c("mx250", "rtx 2070 super"), col = c("black", "green"), lwd = 5, xpd = TRUE, horiz = TRUE, cex = 1, seg.len=1, bty = 'n')
```
