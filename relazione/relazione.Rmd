...
Per fare ciò è stata implementata una reduction (sempre intra-warp a due fasi perché non esiste *atomicMax* per i valori double) per ottenere il valore massimo di un vettore. Se il valore massimo di un vettore è $\leq 0$, allora devono per forza esserlo anche tutti gli altri.
...
```{r, echo=FALSE, warning=FALSE}
  ...
  plot(data = phaseOneMean_ElemNumber, log(values)~log(cells), main = "Fase 1, scala logaritmica", xlab = "Dim. problema (n°vars*n°constr.)", ylab = "Tempo medio solve", type = 'n')
  ...
  for(value in values){
    ...
    plot(data = phaseTwoMean_ElemNumber, log(values)~log(cells), main = "Fase 2, scala logaritmica", xlab = "Dim. problema (n°vars*n°constr.)", ylab = "Tempo medio solve", type = 'n')
  ...
  par(mfrow=c(1,1))
```
Nelle ascisse si trova la dimensione del problema, intesa come n°variabili * n°vincoli (scalata logaritmicamente), mentre nelle ordinate si trova il tempo medio per ciclo di solve speso per tale problema.
...
Per concludere inoltre possiamo anche affermare che il tempo medio non dipende dalla dimensione del problema calcolata come n°variabili * n°vincoli ma da un'altra dimensione, che vedremo in seguito essere quella del tableau.
...
Questa sezione si analizzerà come l'algoritmo scala utilizzando 2 gpu, una mx 250 ed una RTX 2070super, che hanno diverse caratteristiche e performance, riportate nella seguente tabella:
...
[https://personal.utdallas.edu/~scniu/OPRE-6201/OPRE-6201_Course-Content.html](https://personal.utdallas.edu/~scniu/OPRE-6201/OPRE-6201_Course-Content.html)

2. COUTINHO, Demetrios, et al. A Scalable Shared-Memory Parallel Simplex for Large-Scale Linear Programming. arXiv preprint arXiv:1804.04737, 2018.