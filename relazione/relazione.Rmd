...
Queste operazioni servono a riportare i problemi in una forma che permetta loro di essere utilizzati da parte dei solver.
...
Si basa sul teorema secondo il quale le soluzioni (dette *soluzioni di base*) di un qualsiasi problema in forma standard sono i *vertici* del poliedro costruito sulle rette rappresentate dai vincoli.
...
-   Siccome l'algoritmo opera spesso su singole righe o colonne è fondamentale memorizzare i dati in memoria globale in modo da minimizzare il numero di accessi strided o disallineati.
    ...
    -   il vettore dei costi non viene memorizzato nella matrice del tableau, ma in un vettore a parte e il valore della funzione obiettivo è nel primo elemento di questo vettore (analogalmente a quanto fatto con il vettore dei termini noti al punto successivo).

    -   il vettore dei termini noti non viene memorizzato in fondo al tableau, ma nella prima riga della matrice.
        In questo modo nel passaggio dalla fase 1 alla fase 2 non è necessario spostare la colonna (o, in questo caso, la riga) nella matrice, ma è sufficiente troncare le righe relative alle variabili artificiali da una variabile del programma in C senza dover intervenire sull'allocazione della memoria (stesso discorso vale anche per il vettore dei costi);
...
Per fare ciò si utilizzano sei CUDA Stream differenti:
...
: Risultati sperimentali dei diversi kernel, con la configurazione di lancio ottimale (blocco: $32\times 32$, grid: (1, $\lceil 24.588/32\rceil$)) ed ottenuti con una scheda Geforce MX250
...
Utilizzando gli stessi seed per i generatori di numeri pseudo-casuali è stato possibilie rieseguire il benchmark su dispositivi diversi e poter confrontare le prestazioni (in quanto vengono generate le stesse istanze).
...
Si può anche notare come la differenza di tempo speso per ***ciclo di solve*** aumenti linearmente all'aumentare della dimensione reale del problema.
...
Attraverso questi grafici si può notare come i tempi necessari per eseguire un ***ciclo di solve*** siano distribuiti per le due schede video. Si nota ovviamente una distribuzione di tempi inferiori per la RTX2070 Super rispetto alla MX250.
...
Pur non essendoci stato un miglioramento dal punto di vista della complessità teorica del ciclo di solve (che rimane lineare sul numero totale di elementi del tableau), l'implementazione ha mostrato un'ottima scalabilità sia da punto di vista delle dimensioni dei problemi sia dal punto di vista dell'hardware che esegue l'algoritmo, in quanto lo speedup ottenuto tra una scheda di fascia bassa (MX250) e una di fascia medio-alta (RTX2070 Super) è consistente e si riflette anche sul tempo totale ad eseguire i problemi.
