...
Per quanto riguarda l'aggiornamento del tableau, per effettuare il calcolo $T[i][j] - \frac{C_p[i]}{p}R_p[j]$ viene utilizzata la funzione *fma* della libreria math di CUDA\footnote{Questa funzione calcola $A*B+C$ in una sola operazione in virgola mobile}. Il calcolo viene così ridotto a due operazioni in virgola mobile (divisione e fma) invece che tre.
...
Nelle ascisse si trova la dimensione del problema, intesa come $n^{\circ}variabili \times n^{\circ}vincoli$, mentre nelle ordinate si trova il tempo medio per ciclo di solve per tale problema. Entrambe le dimensioni sono in scala logaritmica.
